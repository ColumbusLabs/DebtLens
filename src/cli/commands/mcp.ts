...
import { parseFormat } from "../parse.js";
...
export function runMcpServer(entrypoint = process.argv[1], execArgv = process.execArgv): void {
  ...
  process.stdin.on("data", (chunk) => {
    ...
    while (newline !== -1) {
      ...
      if (line) void handleLine(line, entrypoint, execArgv);
      newline = buffer.indexOf("\n");
    }
  });
}

async function handleLine(line: string, entrypoint: string, execArgv: string[]): Promise<void> {
