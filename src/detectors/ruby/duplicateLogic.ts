...
import { extractRubyFunctions, fingerprintRuby, normalizeRubySnippet } from "./parse.js";

interface RubySnippet {
  name: string;
  file: string;
  startLine: number;
  endLine: number;
...
export const rubyDuplicateLogicDetector: Detector = {
  id: "ruby-duplicate-logic",
  name: "Ruby duplicate logic",
  description: "Finds near-duplicate Ruby methods after comments, names, strings, and literals are normalized.",
  defaultSeverity: "medium",
  tags: ["ruby", "duplication", "maintainability"],
  languages: ["ruby"],
  detect(context: DetectorContext): DebtIssue[] {
    ...
    for (const file of context.files) {
      for (const fn of extractRubyFunctions(file)) {
        ...
        snippets.push({
          ...
          fingerprint: fingerprintRuby(normalized),
        });
      }
    }

    if (snippets.length > maxSnippets) {
      context.addWarning(`ruby-duplicate-logic inspected ${maxSnippets} of ${snippets.length} eligible snippets because duplicate-logic.maxSnippets is capped.`);
    }
    ...
    for (const { leftIndex, rightIndex } of candidatePairs) {
      ...
      issues.push(createIssue({
        detector: rubyDuplicateLogicDetector,
        ...
        suggestion: "Compare the two Ruby implementations. Extract shared behavior only if the variation is intentional and stable; otherwise delete the weaker duplicate.",
      }));
